...
export function prepareCheckAlert(
    getHumidnessStats:
        (site: string, sinceDate: Date)
            => Promise<{ min: number | null; max: number | null; count: number }>,
    getAlert:
        (site: string)
            => Promise<{ timestamp: string | null } | null>,
    setAlert:
        (site: string, date: Date)
            => Promise<void>,
    removeAlert:
        (site: string)
            => Promise<void>,
    ...
    return async (measurement: NewMeasurement): Promise<boolean> => {
        ...
        if (relativeMoisture < RELATIVE_MOISTURE_THRESHOLD) {
            const now = Date.now();
            const lastAlert = await getAlert(measurement.site);

            if (!lastAlert || !lastAlert.timestamp) {
                setAlert(measurement.site, new Date());
                return true
            }
            const lastAlertDate = new Date(lastAlert.timestamp);

            if (now - lastAlertDate.getTime() > ALERT_COOLDOWN_MS) {
                setAlert(measurement.site, new Date(measurement.timestamp));
                return true
            }
        } else if (relativeMoisture > RELATIVE_MOISTURE_THRESHOLD + 10) {
            // 十分に水があると判断してアラートを解除
            removeAlert(measurement.site);
        }
        return false
    }
}
...
export function addMeasurementRoute(app: Elysia, db: Database, commands: typeof sqliteCommands) {
    const { insertMeasurement, getHumidnessStats, getAlert, setAlert, removeAlert } = commands(db);
    app.post("/", async ({ body }) => {
        try {
            ...
            const checkAlert = prepareCheckAlert(getHumidnessStats, getAlert, setAlert, removeAlert)
